...
(define (init)
  ...
  (set-char-texture *char-tex* #\A (~ *tex* 0))
  (set-char-texture *char-tex* #\B (~ *tex* 1))
  (set-char-texture *char-tex* #\C (~ *tex* 2) 1.0 1.0  0.5 1.0 0.25 0.0)
  (set-char-texture *char-tex* #\D (~ *tex* 2) 1.0 1.0 -0.5 1.0 0.75 0.0))

;; 画面表示
(define (disp)
  (gl-clear (logior GL_COLOR_BUFFER_BIT GL_DEPTH_BUFFER_BIT))
  ;; 文字に割り付けたテクスチャの一括表示
  (textscrn-disp-texture *char-tex* *tscrn1* 0 0 *width* *height* (win-w *win* 50) (win-h *win* 50))
  ;; テクスチャ付き長方形の表示
