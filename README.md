...
2. shooting.scm で Ctrlキーを使用していますが、環境によっては、  
   ...
   (元々、3個のキーの同時押しが効かないキーボードがあり、  
   Ctrlキーなら認識できたため、割り当てを追加したのだが。。。)  
   → Windows については、Gauche-gl の開発最新版をコンパイルしてインストールすれば、  
   同梱の freeglut v3.0.0-1 により、Ctrlキー単独の状態が取れるようになる。  
   (freeglut の v2.8.0 以後で対応されたもよう)  
   (2016-2-12)
